...
export const applicationApi = {
  ...
  async getCompanyApplicationsByPosting(postingId: number) {
    try {
      const { data } = await apiClient.get<CompanyApplicationListItem[]>(
        `/api/company/applications/posting/${postingId}`
      )
      return data
    } catch (e) {
      const error = parseAxiosError(e)
  ...
  async getCompanyApplicationDetail(applicationId: number) {
    try {
      const { data } = await apiClient.get<CompanyApplicationDetail>(
  ...
  /** 직원 등록 (회사가 사용자를 직원으로 등록) */
  async registerEmployee(employeeId: number) {
    try {
      const { data } = await apiClient.post('/api/company/manage/employees', {
        employeeId: employeeId,
      })
      return data
    } catch (e) {
      const error = parseAxiosError(e)
      console.error('[applicationApi.registerEmployee]', error)
      throw error
    }
  },

  /** 회사용: 지원 결과 결정 (합격/불합격) */
  async updateApplicationResult(applicationId: number, status: 'HIRED' | 'REJECTED') {
    try {
      const { data } = await apiClient.patch(
        `/api/company/applications/${applicationId}/result?status=${status}`
      )
      return data
    } catch (e) {
      const error = parseAxiosError(e)
      console.error('[applicationApi.updateApplicationResult]', error)
      throw error
    }
  },
}
